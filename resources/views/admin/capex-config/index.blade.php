@extends('layouts.app')
@section('title', 'CapEx Layouts')
@section('content')

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="fw-bold mb-0">CapEx Layouts</h5>
        <p class="text-muted small mb-0">
            Configure how the Capital Expenditure report is structured.
            Only one layout can be active at a time.
        </p>
    </div>
    <a href="{{ route('admin.capex-configs.create') }}"
...
<div class="row g-3">
    ...
    <div class="col-md-6 col-lg-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius:12px;overflow:hidden">
            <div style="height:4px;background:{{ $config->is_active ? '#E65C00' : '#E2E8F0' }}"></div>
            <div class="card-body p-4">
                <div class="d-flex align-items-start justify-content-between mb-2">
                    <h6 class="fw-bold mb-0" style="color:#1B2A4A">{{ $config->name }}</h6>
                    @if($config->is_active)
                    <span class="badge" style="background:#FFF7ED;color:#C2410C;font-size:10px;border-radius:20px">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('admin.capex-configs.edit', $config) }}"
                       class="btn btn-sm"
                    ...
                    <form method="POST" id="deactivateConfig{{ $config->id }}"
                          action="{{ route('admin.capex-configs.deactivate', $config) }}">
                        @csrf
                    ...
                    <form method="POST" id="activateConfig{{ $config->id }}"
                          action="{{ route('admin.capex-configs.activate', $config) }}">
                        @csrf
                        <button type="button" class="btn btn-sm"
                                style="background:#FFF7ED;color:#C2410C;border-radius:7px;font-size:12px;border:none"
                    ...
                    <form method="POST"
                          action="{{ route('admin.capex-configs.destroy', $config) }}"
...
<div class="mt-4 p-3 rounded-3 small text-muted" style="background:#F8FAFC;border:1px solid #E2E8F0">
    ...
    When a layout is <strong>active</strong>, the Capital Expenditure report renders using
    that layout instead of the default category grouping.
...
function confirmActivate(id, name) {
    Swal.fire({
        ...
        html: `<span class="text-muted">This will make <strong>${name}</strong> the active CapEx layout and deactivate any currently active one.</span>`,
        icon: 'question',
...
function confirmDeactivate(id, name) {
    Swal.fire({
        ...
        html: `<span class="text-muted">The standard CapEx view will be used instead of <strong>${name}</strong>.</span>`,
        icon: 'warning',
        showCancelButton: true,
