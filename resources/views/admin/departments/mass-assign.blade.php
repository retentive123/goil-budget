@extends('layouts.app')
@section('title', 'Mass Assign Account Codes — Departments')
@section('content')
...
<div class="mb-4">
    ...
    <p class="text-muted small mb-0">Select multiple departments and the account codes to assign to all of them at once.</p>
</div>

<form method="POST" action="{{ route('admin.departments.mass-assign.store') }}" id="massForm">
@csrf
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm" style="border-radius:12px; position:sticky; top:80px; max-height:calc(100vh - 160px); display:flex; flex-direction:column;">
            ...
            <div class="px-4 py-2 d-flex justify-content-between align-items-center"
                ...
                <button type="button" class="btn btn-link btn-sm p-0 text-decoration-none" style="font-size:12px; color:#E65C00;" onclick="selectAllDepts()">Select all</button>
                <button type="button" class="btn btn-link btn-sm p-0 text-decoration-none" style="font-size:12px; color:#64748B;" onclick="clearAllDepts()">Clear</button>
            </div>

            <div style="overflow-y:auto; flex:1;">
                @forelse($departments as $dept)
                <label class="dept-row d-flex align-items-center gap-3 px-4 py-2"
                    ...
                    <input type="checkbox" name="department_ids[]" value="{{ $dept->id }}"
                           class="form-check-input dept-cb flex-shrink-0"
                    ...
                    <div style="min-width:0;">
                        ...
                        <div style="font-size:11px; color:#94A3B8;">{{ $dept->code }}</div>
                    </div>
                </label>
                @empty
...
<div style="position:fixed; bottom:0; left:0; right:0; background:#fff; border-top:2px solid #E2E8F0; z-index:100; padding:14px 24px;">
    <div class="d-flex align-items-center gap-4 flex-wrap">
        ...
        <div style="font-size:13px; color:#475569;">
            <span id="barDepts" class="fw-bold" style="color:#E65C00;">0</span> departments ×
...
function updateDeptCount() {
    const n = document.querySelectorAll('.dept-cb:checked').length;
    document.getElementById('deptBadge').textContent = n + ' selected';
    document.getElementById('barDepts').textContent  = n;
    updateSubmit();
}
...
function updateSubmit() {
    ...
    btn.disabled    = !(depts > 0 && codes > 0);
    btn.style.opacity = (depts > 0 && codes > 0) ? '1' : '.5';
}

function selectAllDepts() {
    document.querySelectorAll('.dept-cb').forEach(cb => cb.checked = true);
    updateDeptCount();
}
...
document.addEventListener('change', function(e) {
    ...
    if (e.target.classList.contains('dept-cb')) {
...
<style>
.dept-row:hover { background: #FFF7ED !important; }
