...
/* Unchecked: GOIL orange border */
.form-check-custom .form-check-input {
    border: 2px solid #E65C00;
    margin-top: 1px;
}
.form-check-custom .form-check-input:focus {
    box-shadow: 0 0 0 3px rgba(230,92,0,.15);
    border-color: #E65C00;
}
...
/* Card when checked: green border + tint */
...
<script>
    ...
    // Tab type map — null means show all
    const TAB_TYPES = {
        'all':                null,
    ...
    function applyFilters() {
        ...
        document.querySelectorAll('.category-group').forEach(group => {
            const gType = group.dataset.type || '';
            if (tabTypes && !tabTypes.includes(gType)) {
            ...
            group.querySelectorAll('.code-item').forEach(item => {
                const match = !query
                    || (item.dataset.code || '').includes(query)
