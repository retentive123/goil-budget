<tr>
    <td style="padding:10px 16px">
        <div style="font-size:13px;font-weight:600;color:#1B2A4A">{{ $dept->name }}</div>
        <div style="font-size:10px;color:#64748B">{{ $dept->code }}</div>
    </td>
    <td>
        <span style="padding:2px 8px;border-radius:20px;font-size:10px;font-weight:600;
                     background:{{ match($dept->budget_type) {
                         'revenue' => '#D1FAE5', 'both' => '#DBEAFE', default => '#FEE2E2'
                     } }};
                     color:{{ match($dept->budget_type) {
                         'revenue' => '#065F46', 'both' => '#1E40AF', default => '#991B1B'
                     } }}">
